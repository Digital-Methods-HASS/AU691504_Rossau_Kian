...
- Use the `name()` function and assignment key to relabel this column to `homicides_per_100k` 
...
```{r plot the data of Western Europe, eval = TRUE}
#pipe
Western_Europe %>%  
  ggplot(aes(x = Year, y = homicides_per_100k, color = Country))+
  geom_col()+
labs(title = "Homicide in Western Europe", x = "years", y = "Homicides par 100k")
```
...
```{r split_the_data_into_nations ,eval = TRUE}
Western_Europe %>%  
  ggplot(aes(x = Year, y = homicides_per_100k, color = Country))+
  facet_wrap(~ Country, nrow = 2)+
  geom_line()+
labs(title = "Homicide in Western European nations", x = "years", y = "Homicides par 100k")
```
...
```{r the Danish king dataset ,eval = TRUE}
library(tidyverse)
library(here)
Kings <- read_csv(here("data", "Danish_kings.csv"),na= "NULL")
Kings$birthdate = as.numeric(Kings$birthdate)
Kings$Deathdate = as.numeric(Kings$Deathdate)
Kings %>% 
  mutate(Midtyear = Deathdate - ( Deathdate - birthdate)/2) %>% 
  ggplot(aes(x = Midtyear, y = duration_of_reign))+
    geom_smooth()+
  labs(title = "Duration of reign for danish kings", x = "years", y = "Reign")
```
I would characterize the trend with danish kings to be that in generel Kings reign for longer time as closer to modern day we arrive, which it fit a bit with homicide in Europe as there is a decline of homicide around the time as there is a incline in how long kings reign.  
...
A good question and it depends on what period we are in compared with our time. 
When looking at the plot of the data as a whole we can see a massive drop in Homicides between the numbers now and back in the 1400s, however, it is not 100% as there are less accurate data compared to now. 
Looking closer to modern times in the plot we can see that at some point around the 1900s the drop in Homicides stops and it is more or less the same as the level we have in the 2000s. 
When taking a closer look at each nation in the split-plot part, many trends are showing, first is that every nation at some point just keeps having the same level of homicides, but at different times in history. Another thing that may or may not be relevant is that Italy and the Netherlands have a high amount of homicides in the 1400s compared to the other nations, which may be because of more data from those nations or those nations having a higher population or urban population in that time. 
A Funny thing is that the number of homicides in Italy get an increase in the 1800s. 
The trend of the lower amount of homicides fit also together with the trend of the danish kings dataset, where the reign of kings increases in the number of years as more modern we get. 
From the data show, then yes we are more civilized today in the sense that we do not murder each other as much as we did in the past 
